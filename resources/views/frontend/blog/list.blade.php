                ...
                <div class="container">
                    ...
                    <div class="tts-blog-content">
                        <div class="clearfix mb-3">
                            <form action="{{ route('public.search.blog') }}" method="GET">
                                ...
                                <div class="tts-blog-content-category">
                                    @foreach ($category as $item)
                                        <div class="tts-blog-category-item active">
                                            <a href="{{ route('frontend.get.category.post', $item->id) }}">{{ $item->name }}</a>
                                            @if(isset($id) && $item->id == $id)
                                ...
                                <div class="tts-blog-content-right">
                                    ...
                                    @foreach ($posts_menu as $item)
                                        <div class="tts-item-blog-menu">
                                            <p onclick="javascript:location.href='{{ route('frontend.get.post', $item->id) }}'" style="cursor: pointer">{{ $item->name }}</p>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                                    ...
                                    @foreach ($posts as $item)
                                        <div class="col-lg-6 tts-col-box-item">
                                            <div class="tts-item-event-blog" style="height: 100%">
                                                <a href="{{ route('frontend.get.post', $item->id) }}">
                                                    <img class="img-fluid" src="/storage/{{ $item->image }}" onclick="javascript:location.href='{{ route('frontend.get.post', $item->id) }}'" style="height: 300px; object-fit: cover; cursor: pointer" alt="">
                                                </a>
