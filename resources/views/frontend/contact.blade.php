...
@section('main')
    @if($errors->any())
        <div id="box_message" class="pl-3 pr-3 pt-1 pb-1" style="position: fixed; top: 50px; right: 50px; z-index: 9999; width: auto; background-color: #ac5500; border-radius: 5px">
            <h4 class="text-white mb-0 pb-0" style="font-size: 16px">{{$errors->first()}}</h4>
        </div>
    @endif
    @php
                ...
                <div class="row">
                    ...
                    <div class="col-lg-4">
                        <div class="tts-contact-box-item-info">
                            ...
                            <P>{{ $info->abbot_phone }}</P>
                            <P class="mb-0">Văn Phòng</P>
                            <P>{{ $info->office_phone }}</P>
                            <P class="mb-0">Thư Ký</P>
                            <P>{{ $info->secretary_phone }}</P>
                        </div>
                    </div>
                    <div class="col-lg-4">
                        <div class="tts-contact-box-item-info">
                            <h3>THÔNG TIN NGÂN HÀNG</h3>
                            <p>CHỦ TK: {{ $info->Bank_account }}</p>
                            <P>STK: {{ $info->Bank_number }}</P>
                            <P>{{ $info->Bank_nane }}</P>
                        </div>
                    </div>
                    <div class="col-lg-6 offset-lg-3 mt-5">
                        <div class="tts-contact-box-item-info">
                            ...
                            <form action="{{ route('frontend.post.newsletter') }}" method="post">
                                @csrf
